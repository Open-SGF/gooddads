...
interface ServicePlanFormDefinition
...
export const ServicePlanForm: React.FC<ServicePlanFromProps> = ({
	...
	const { data, setData, processing, errors, ...form } =
		useForm<ServicePlanFormDefinition>({
			participant_name:
				serviceplanForm?.participant_name ?? participant?.name ?? '',
			...
			parenting_skill_development_is_service_area:
				serviceplanForm?.parenting_skill_development_is_service_area ?? true,
			effective_co_parenting_is_service_area:
				serviceplanForm?.effective_co_parenting_is_service_area ?? true,
			employment_and_education_is_service_area:
				serviceplanForm?.employment_and_education_is_service_area ?? true,
			child_support_is_service_area:
				serviceplanForm?.employment_and_education_is_service_area ?? true,
			domestic_violence_is_service_area:
				serviceplanForm?.domestic_violence_is_service_area ?? false,

			service_identified_by_participant:
				serviceplanForm?.service_identified_by_participant ?? '',
			goal: serviceplanForm?.goal ?? '',

			custody_visitation_strategy:
				serviceplanForm?.custody_visitation_strategy ?? '',
			custody_visitation_person_responsible:
				serviceplanForm?.custody_visitation_person_responsible ?? '',
			custody_visitation_timeline:
				serviceplanForm?.custody_visitation_timeline ?? '',
			custody_visitation_measure_of_success:
				serviceplanForm?.custody_visitation_measure_of_success ?? '',
			education_employment_strategy:
				serviceplanForm?.education_employment_strategy ?? '',
			education_employment_person_responsible:
				serviceplanForm?.education_employment_person_responsible ?? '',
			education_employment_timeline:
				serviceplanForm?.education_employment_timeline ?? '',
			education_employment_measure_of_success:
				serviceplanForm?.education_employment_measure_of_success ?? '',
			housing_transportation_strategy:
				serviceplanForm?.housing_transportation_strategy ?? '',
			housing_transportation_person_responsible:
				serviceplanForm?.housing_transportation_person_responsible ?? '',
			housing_transportation_timeline:
				serviceplanForm?.housing_transportation_timeline ?? '',
			housing_transportation_measure_of_success:
				serviceplanForm?.housing_transportation_measure_of_success ?? '',
			participant_signature: serviceplanForm?.participant_signature ?? '',
			participant_signature_date:
				serviceplanForm?.participant_signature_date ?? '',
			case_manager_signature: serviceplanForm?.case_manager_signature ?? '',
			case_manager_signature_date:
				serviceplanForm?.case_manager_signature_date ?? '',
			...
			<form
				onSubmit={(e) => {
					...
					if (serviceplanForm?.id) {
						form.put(
							route('intake.service-plan.update', [serviceplanForm.id]),
							{
								onSuccess: () => {
									continueToNextStep()
								},
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-2">
						<div className="flex items-center">
							...
							<Label
								...
								htmlFor="parenting_skill_development"
							>
						...
						<div className="flex items-center">
							<Checkbox
								...
								onCheckedChange={(checked) =>
									setData('effective_co_parenting_is_service_area', !!checked)
								}
								disabled={viewOnly || processing}
							/>
							<Label
								className="ml-2 block text-gray-700"
								htmlFor="effective_co_parenting"
							>
						...
						<div className="flex items-center">
							<Checkbox
								...
								onCheckedChange={(checked) =>
									setData('employment_and_education_is_service_area', !!checked)
								}
								disabled={viewOnly || processing}
							/>
							<Label
								className="ml-2 block text-gray-700"
								htmlFor="employment_and_education"
							>
						...
						<div className="flex items-center">
							<Checkbox
								...
								onCheckedChange={(checked) =>
									setData('child_support_is_service_area', !!checked)
								}
								disabled={viewOnly || processing}
							/>
							<Label
								className="ml-2 block text-gray-700"
								htmlFor="child_support"
							>
								Child Support
							</Label>
						</div>
						...
						<div className="flex items-center">
							<Checkbox
								...
								onCheckedChange={(checked) =>
									setData('domestic_violence_is_service_area', !!checked)
								}
								disabled={viewOnly || processing}
							/>
							<Label
								className="ml-2 block text-gray-700"
								htmlFor="domestic_violence"
							>
								Domestic violence
							</Label>
						</div>
				...
				<div className="mb-6">
					<div className="mb-4">
						...
						<Input
							...
							onChange={(e) =>
								setData('service_identified_by_participant', e.target.value)
							}
							disabled={viewOnly || processing}
						/>
					...
					<div>
						...
						/>
						{errors.goal && (
							<div className="text-red-500 text-xs">{errors.goal}</div>
						)}
					</div>
				</div>
					...
					<div className="overflow-x-auto mb-8">
						<table className="min-w-full bg-white">
							...
							<tbody>
								<tr>
									<td className="py-2 px-4 border">
										Custody/Visitation<span className="text-red-500">*</span>
									</td>
									<td className="py-2 px-4 border">
										<Input
											type="text"
											...
											onChange={(e) =>
												setData('custody_visitation_strategy', e.target.value)
											}
											disabled={viewOnly || processing}
										/>
									...
									<td className="py-2 px-4 border">
										<Input
											...
											onChange={(e) =>
												setData('custody_visitation_timeline', e.target.value)
											}
											disabled={viewOnly || processing}
										/>
								...
								<tr>
									...
									<td className="py-2 px-4 border">
										<Input
											...
											onChange={(e) =>
												setData('education_employment_strategy', e.target.value)
											}
											disabled={viewOnly || processing}
										/>
									...
									<td className="py-2 px-4 border">
										<Input
											...
											onChange={(e) =>
												setData('education_employment_timeline', e.target.value)
											}
											disabled={viewOnly || processing}
										/>
					...
					<div className="overflow-x-auto mb-8">
						<table className="min-w-full bg-white">
							...
							<tbody>
								<tr>
									<td className="py-2 px-4 border">
										Child Support Awareness and Information
									</td>
									<td className="py-2 px-4 border">
										<Input
											type="text"
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
						<div>
							...
							<Input
								...
								onChange={(e) =>
									setData('participant_signature', e.target.value)
								}
								disabled={viewOnly || processing}
							/>
						...
						<div>
							<label className="block text-gray-700 text-sm font-bold mb-2">
								Case Manager Signature:<span className="text-red-500">*</span>
							</label>
							<Input
								type="text"
								className="w-full p-1 border border-gray-300 rounded"
								...
								onChange={(e) =>
									setData('case_manager_signature', e.target.value)
								}
								disabled={viewOnly || processing}
							/>
